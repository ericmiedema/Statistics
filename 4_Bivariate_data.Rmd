...
The sum of the product of Z is `r round(sum(df$product_z),1)` divided by 9 to get r = `r round(sum(df$product_z)/9,2)`
...
**y = a + bx** to bivariate data $(x1, y1), (x2, y2),(xn,yn)$ is the sum of the of the squared deviations about the line.
...
## Reciprical Transformation Example

```{r echo = FALSE, warning=FALSE, message=FALSE}
library(ggplot2)
library(gridExtra)
x<-c(runif(90,min = 1, max = 100),runif(10,min = 0, max = 1))
rand<-x
...
plot1<-ggplot(df, aes(x,y)) + geom_point() + stat_smooth(method="lm", se=TRUE, fill=NA,
                formula= y ~ x,colour="red") + labs(title = paste("Reciprocal of x",sep = " "))
plot2<-ggplot(df, aes(1/x,y)) + geom_point() + geom_smooth(method = "lm") + labs(title = paste("r2 = ",r2,", y = ",a," + ",b,"x", sep = ""))

grid.arrange(plot1, plot2, ncol=2)
```

<div class="notes">
* Quadratic - Models with peak or valley such as sales on new iphone or game
* Reciprocal - curve for high first day or event such as hospital cost per day in the hospital (ED first day)
* Square Root - ???
* Log - curve for improvement such as patient satisfaction
* Exponential
* Power
</div>
